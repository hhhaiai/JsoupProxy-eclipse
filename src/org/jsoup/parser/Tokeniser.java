...
final class Tokeniser {
    ...
    private StringBuilder charsBuilder = new StringBuilder(1024); // buffers
                                                                  // characters
                                                                  // to output
                                                                  // as one
                                                                  // token, if
    ...
    void eofError(TokeniserState state) {
        if (errors.canAddError())
            errors.add(
                    new ParseError(reader.pos(), "Unexpectedly reached end of file (EOF) in input state [%s]", state));
    }
